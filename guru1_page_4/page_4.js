...
$(document).ready(function(){
		...
		$("#do").click(function() { //상단 메뉴 클릭시
      if($(this).parent().find('#do_li').is(":visible")){ //하위 메뉴가 화면상 보일때 (:is visible) slideup
        $(this).parent().find("#do_li").slideUp('normal');
      }
      else{  
				$(this).parent().find("#do_li").slideDown('normal'); //안보일 때 slideDown
			} 
}); 
